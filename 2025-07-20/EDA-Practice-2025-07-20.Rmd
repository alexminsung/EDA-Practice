...
df_plot2 <- full_join(df_plot2, df_INCOME, by = join_by(Transaction.Date))
# rename cumulative Income to INCOME
df_plot2 <- df_plot2 %>% rename(INCOME = cum_value) %>% arrange(Transaction.Date)
...
dygraph(time_series_df_plot2, main = "Overall Spending vs Income (2023-06 to 2025-09)") %>% dyAxis("y", label = "Cumulative Spending (CAD)") %>% dyLegend(show = "follow") %>% dyRangeSelector() %>% dyCSS("dygraph.css") %>% dySeries("INCOME", fillGraph = TRUE, color = "green") %>% dySeries("SPENDING", fillGraph = TRUE, color = "red") 
